...
describe('POST /booking', () => {
  ...
  describe('when token is valid', () => {
    ...
    it("should respond with status 404 with an invalid body - there's not roomId", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType(false, true);
    ...
    it("should respond with status 403 with an invalid body - there's no vacancy", async () => {
      const user1 = await createUser();
      const user2 = await createUser();
      const user3 = await createUser();
      const user4 = await createUser();
      const token = await generateValidToken(user4);
      const enrollment = await createEnrollmentWithAddress(user4);
      const ticketType = await createTicketType(false, true);
      const ticket = await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      await createPayment(ticket.id, ticketType.price);
    ...
    it('should respond with status 403 if user has not enrolled', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
